...
import {listingToText, embedText, toVectorLiteral} from "../../utils/embedding.util";
...
export const listingResolvers = {
    ...
    Query: {
        ...
        getListings: async (_parent: any, args: {
            q?: string,
            type?: string,
            category?: string,
            sort?: string,
        ...
        getSimilarListings: async (
            ...
            checkStudentOnly(context);
            const limit = Math.min(Math.max(1, args.limit ?? 8), 20);

            // kaynak ilanın kendi koordinatına en yakın, aynı tipteki aktif ilanlar
            const rows = await context.prisma.$queryRaw<{ id: string }[]>`
                SELECT l.id
                FROM "Listing" l
                WHERE l.id <> ${args.id}
                  AND l.status = 'active'
                  ...
                  AND l.type = (SELECT type FROM "Listing" WHERE id = ${args.id})
                  AND (l.expires IS NULL OR l.expires > NOW())
                ORDER BY l.embedding <=> (SELECT embedding FROM "Listing" WHERE id = ${args.id})
                    LIMIT ${limit}
            `;
            ...
            // sıralamayı koruyarak tam ilan objelerini çek (owner vb. field resolver'lar çalışsın)
            const listings = await context.prisma.listing.findMany({where: {id: {in: ids}}});
    ...
    Mutation: {
        ...
        createListing: async (_parent: any, args: { input: any }, context: GraphQLContext) => {
            ...
            // ── embedding üret ve yaz (hata olursa ilan oluşturma bozulmasın) ──
            try {
                const vec = await embedText(listingToText(newListing as any));
                await context.prisma.$executeRaw`
                    UPDATE "Listing"
                    SET embedding = ${toVectorLiteral(vec)}::vector
                    WHERE id = ${newListing.id}`;
            } catch (e) {
                console.error("Embedding üretilemedi:", e);
            }
