...
export default function CreateEmergencyPage() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        console.log("Emergency Listing Payload:", { title, location, description, duration: selectedDuration });
      ...
      `}} />
            ...
            <div className="w-full max-w-2xl mx-auto bg-black/40 backdrop-blur-xl border border-rose-500/20 rounded-[2rem] p-8 md:p-10 shadow-[0_0_50px_rgba(244,63,94,0.05)] z-10 relative">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            {durations.map((dur) => (
                                ...
                                >
                                    <span className={`font-bold ${selectedDuration === dur.id ? 'text-rose-400' : 'text-gray-300'}`}>{dur.label}</span>
                                    <span className={`text-[10px] uppercase tracking-wider mt-1 ${selectedDuration === dur.id ? 'text-rose-300/70' : 'text-gray-600'}`}>{dur.desc}</span>
                                </button>
                            ))}
                        </div>
                        <p className="text-xs text-gray-500 ml-1 mt-2">Süre dolduğunda ilan otomatik olarak yayından kalkar ve silinir.</p>
                    </div>
                    ...
                    >
                        <span>{isFormValid ? 'ACİL İLAN YAYINLA' : 'İHTİYACINI VE KONUMUNU YAZ'}</span>
                        {isFormValid && <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />}
                    </button>
